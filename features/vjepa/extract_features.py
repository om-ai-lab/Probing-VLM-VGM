...
python -m features.vjepa.extract_features \
       --scene-dir  data/DL3DV/DL3DV-10K/1K/<hash>/images_4 \
       --data-sft   data/DL3DV/DL3DV-processed/1K/<hash>.sft \
       --out-dir    features/vjepa/1K/<hash> \
...
def main(argv: list[str] | None = None) -> None:
    ...
    p.add_argument("--checkpoint", default="features/vjepa/vith16-384.pth.tar")
    ...
    if start_idx + window_size <= total:
        ...
        # Short scene: pad by repeating the last frame so V-JEPA sees a full
        # window. Query-GT alignment stays correct because dataset clamps
        # effective_context_len <= gt_num_frames <= total.
        pad = window_size - total
        logging.warning(
            f"[VJEPA] {os.path.basename(args.scene_dir)}: only {total} frames "
