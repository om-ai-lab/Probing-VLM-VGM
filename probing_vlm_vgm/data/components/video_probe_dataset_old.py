...
class VideoProbeDataset(EasyDataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.gt_num_frames = gt_num_frames  # uniformly downsample GT to this many frames
        assert (
    ...
    def __getitem__(self, idx):
        ...
        # Compute downsampling indices BEFORE converting to float32,
        # so we only convert the frames we actually need (e.g. 16 out of 150).
        orig_num_frames = data["images"].shape[0]
        if self.gt_num_frames is not None and self.gt_num_frames < orig_num_frames:
            si = torch.linspace(
                0, orig_num_frames - 1, self.gt_num_frames
            ).round().long()
        else:
            si = slice(None)

        # Index FIRST, then .float() — avoids allocating float32 copies of all
        # 150 frames when only gt_num_frames are needed.
        # Keep images in original dtype (uint8); the / 255.0 later auto-promotes.
        images = data["images"][si]  # (S', 3, H, W), still uint8
        pointmaps = data["pointmaps"][si].float()  # (S', H, W, 3)
        confmaps = data["confmaps"][si].float()  # (S', H, W, 1)
        depthmaps = data["depthmaps"][si].float()  # (S', H, W, 1)
        intrinsics = data["intrinsic"][si].float()  # (S', 3, 3)
        extrinsics = data["extrinsic"][si].float()  # (S', 3, 4)
        masks = data["masks"][si] if self.use_mask and "masks" in data else None

        # Release mmap-backed file data immediately
        del data
        ...
        if self.vfm_name in ["wan", "opensora"]:
            ...
            if self.feat_pixalign:
                vfm_feat = vfm_feat[vfm_idx]
                vfm_idx = torch.arange(vfm_feat.shape[0], device=vfm_feat.device)
        elif self.vfm_name in ["internvl", "internvl_sensenova", "qwen3vl", "qwen3vl_sensenova"]:
            # load InternVL features
            # InternVL features can be single-layer or multi-layer
            # feat_postfix specifies which layer(s) to use, e.g., "_layer32" or "_layer8_16_24_32"
            # Single layer: feature_layer{N}.sft with shape (T, H, W, C)
            # Multi-layer: stack multiple layers
            ...
            # InternVL features are extracted per-frame, so direct mapping
            # sel contains frame indices, map to feature indices
            # If T < total_frames, we need to map
            total_frames = images.shape[0]
            if T == total_frames:
                vfm_idx = sel
            else:
                # Map frame indices to feature indices (uniform sampling was used during extraction)
                vfm_idx = torch.floor(sel.float() / (total_frames - 1) * (T - 1)).long()
                vfm_idx = vfm_idx.clamp(0, T - 1)
