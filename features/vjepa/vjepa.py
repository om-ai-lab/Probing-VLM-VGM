...
def build_model(ckpt_path=None):
    model = vit_huge(num_frames=16)
    if not ckpt_path:
        current_dir = os.path.dirname(os.path.abspath(__file__))
        ckpt_path = os.path.join(current_dir, "c.pth.tar")
    state_dict = torch.load(ckpt_path, map_location=torch.device("cpu"))["encoder"]
    matched_state_dict = {}
    for key in state_dict.keys():
        matched_state_dict[key[16:]] = state_dict[key]
