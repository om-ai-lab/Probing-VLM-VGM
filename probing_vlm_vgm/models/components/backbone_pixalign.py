...
from probing_vlm_vgm.models.components.patchify import PatchifyTokenizer
from probing_vlm_vgm.vggt.layers.block import Block
from probing_vlm_vgm.vggt.layers.rope import PositionGetter, RotaryPositionEmbedding2D
...
class BackbonePA(nn.Module):
    ...
    Args:
        ...
        dropout (float): proj + MLP dropout applied inside every block.
    """
    ...
    def __init__(
        ...
        # depth=0 → "read-out purity" mode (Abl-1). The backbone reduces to
        # input_proj + (camera + register) token prepending, with no attention
        # layers. forward() returns an output_list of 4 identical references
        # so DPT-style multi-stage heads still work; semantic / instance heads
        # consume the same single tensor. Heads that depend on intra-token
        # mixing (camera regression) become degenerate at depth=0 — this is
        # an intended part of the ablation, not a bug.
        # `dropout` flows into Block as `drop=` → applied as proj_drop on the
        # attention output projection AND as the MLP dropout. attn_drop (the
        # dropout on attention weights themselves) is intentionally left at 0;
        # `drop` is the standard transformer dropout knob.
        ...
        if self.depth == 0:
            # No alt-attn loop → aa_block_num is conceptually 1 (one "pass"
            # that does nothing). Keep this for downstream code that asks how
            # many output stages we produce.
            self.aa_block_num = 1
        else:
    ...
    def forward(
        ...
        # depth=0 shortcut: no attention layers. Emit 4 identical "stages" so
        # the DPT head's multi-scale fusion (which expects len(output_list) >=
        # max(intermediate_layer_idx)+1) still works. The 2*C channel layout
        # mirrors the concat(frame, global) used in the regular path, so head
        # signatures don't need to change.
        if self.depth == 0:
            stage_tokens = tokens.view(B, S, P, C)
            stage_2c = torch.cat([stage_tokens, stage_tokens], dim=-1)  # (B,S,P,2C)
            output_list = [stage_2c, stage_2c, stage_2c, stage_2c]
