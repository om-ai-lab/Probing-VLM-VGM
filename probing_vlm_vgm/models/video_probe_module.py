...
from probing_vlm_vgm.eval.finegrained_metrics import all_finegrained_for_sample
...
from probing_vlm_vgm.utils.eval_utils import align_pmaps, build_mask, pmaps_to_pc
from probing_vlm_vgm.utils.vis_utils import dump_viser_artifact, save_scene_glb, vfm_pca_images
...
class VideoProbeLitModule(BaseProbeModule):
    def __init__(
        self,
        ...
        save_pointcloud: bool = True,  # if False, skip saving .ply and .glb files
        skip_test_viz: bool = False,  # if True, skip per-batch PCA/grid/viser in test_step (metrics still computed)
        ...
        **kwargs: Any,
        ...
        self.case_output_key = kwargs.get("case_output_key", "scene")
        self.case_viz_scene_ids = self._parse_case_viz_scene_ids(
            kwargs.get("case_viz_scene_ids", "")
        )
        self.case_viz_batch_samples = self._parse_case_viz_batch_samples(
            kwargs.get("case_viz_batch_samples", "")
        )
        ...
        self.skip_test_viz = skip_test_viz
    ...
    @staticmethod
    def _parse_case_viz_scene_ids(raw: Any) -> set:
        if not raw:
            return set()
    ...
    @staticmethod
    def _parse_case_viz_batch_samples(raw: Any) -> set:
        if not raw:
            return set()
        if isinstance(raw, str):
        ...
        for item in items:
            if ":" not in item:
                continue
            batch_idx, sample_idx = item.split(":", 1)
            pairs.add((int(batch_idx), int(sample_idx)))
        return pairs
    ...
    def _case_output_dir(
        ...
        if not self.case_output_root:
            return None
        if self.case_output_key == "batch_sample" and batch_idx is not None:
            scene_id = f"batch_{int(batch_idx):04d}_sample_{int(sample_idx):02d}"
        else:
            scene_id = self._case_scene_id(data, sample_idx)
        model_name = self.case_model_name or os.path.basename(str(self.output_path or "model"))
        return os.path.join(str(self.case_output_root), scene_id, model_name)

    def _case_scene_id(self, data: Dict[str, torch.Tensor], sample_idx: int) -> str:
        scene_paths = data.get("scene_path", None)
        if isinstance(scene_paths, (list, tuple)) and sample_idx < len(scene_paths):
            scene_path = scene_paths[sample_idx]
        else:
            scene_path = f"batch_{sample_idx}"
        return os.path.splitext(os.path.basename(str(scene_path)))[0]

    def _should_save_case_viz(
        self, data: Dict[str, torch.Tensor], sample_idx: int, batch_idx: int
    ) -> bool:
        if not self.case_viz_scene_ids and not self.case_viz_batch_samples:
            return True
        scene_id = self._case_scene_id(data, sample_idx)
        return (
            scene_id in self.case_viz_scene_ids
            or (int(batch_idx), int(sample_idx)) in self.case_viz_batch_samples
        )
    ...
    def on_validation_epoch_start(self) -> None:
        # Base class handles set_epoch on dataset/sampler; we add metric reset.
        ...
        if (
            ...
            for i in range(batch_size):
                ...
                if "pmaps" in preds:
                    ...
                    if self.global_step % self.train_viz_freq == 0 and self.save_pointcloud:
        ...
        if (
            ...
            for i in range(batch_size):
                ...
                if batch_idx < 2 and i < 2:
                    ...
                    if "pmaps" in preds and self.save_pointcloud:
                        self.visualize_pmaps(
    ...
    def test_step(
        ...
        if not self.skip_test_viz:
            for i in range(batch_size):
                if not self._should_save_case_viz(data, i, batch_idx):
                    continue
                save_path = os.path.join(
                    self.output_path,
                    "viz",
                ...
                case_dir = self._case_output_dir(data, i, batch_idx)
                if case_dir is not None:
                    self.visualize_image_grid(
                        ...
                        gt_pmaps=data["pmaps"][i] if "pmaps" in data else None,
                        pca_map=pca_maps,
                        save_path=os.path.join(case_dir, "geometry_grid.png"),
                        name=f"case-viz/{batch_idx}-{i}:grid",
                        save_individual=True,
                        save_only=True,
                    )
        ...
        if "pmaps" in preds:
            aligned_mses = []
            aligned_preds = []
            fg_metrics_per_sample: List[Dict[str, float]] = []
            for i in range(batch_size):
                mask_i = build_mask(
                    data["cmaps"][i],
                    data["masks"][i] if "masks" in data else None,
                ...
                # Fine-grained metrics (G1 / D3 / C2 / T1). test-time only.
                # Wrapped so a single bad sample never breaks an eval run.
                try:
                    fg_dict = all_finegrained_for_sample(
                        aligned_pred_pmap=pred_aligned,
                        gt_pmap=data["pmaps"][i].detach(),
                        pred_depth=preds["dmaps"][i].detach()
                        if "dmaps" in preds
                        else None,
                        gt_depth=data["dmaps"][i].detach(),
                        valid_mask=mask_i,
                        vfm_idx=data["vfm_idx"][i],
                    )
                    fg_metrics_per_sample.append(fg_dict)
                except Exception as e:
                    log.warning(
                        f"finegrained metrics failed for batch {batch_idx} sample {i}: {e}"
                    )
                    fg_dict = {}
                case_dir = self._case_output_dir(data, i, batch_idx)
                if case_dir is not None:
                    os.makedirs(case_dir, exist_ok=True)
                    model_name = self.case_model_name or os.path.basename(
                        str(self.output_path or "model")
                    )
                    metrics_payload = {
                        "scene_id": self._case_scene_id(data, i),
                        "model": model_name,
                        "batch_idx": int(batch_idx),
                        "sample_idx": int(i),
                        "pmap_mse_aligned": float(aligned_mse.item()),
                    }
                    ...
                    with open(
                        os.path.join(case_dir, "geometry_metrics.json"), "w"
                    ) as f:
                        json.dump(metrics_payload, f, indent=2)
                    metrics_dir = os.path.join(str(self.case_output_root), "_metrics")
                    os.makedirs(metrics_dir, exist_ok=True)
                    safe_model_name = re.sub(r"[^A-Za-z0-9_.-]+", "_", model_name)
                    with open(
                        os.path.join(metrics_dir, f"{safe_model_name}.jsonl"), "a"
                    ) as f:
            ...
            # Aggregate fine-grained dicts across the batch (NaN-aware mean)
            # and emit one scalar per key, mirroring the val/pmap_mse_aligned
            # pattern above so parse_results.py picks them up.
            if fg_metrics_per_sample:
                all_keys = set()
                for d in fg_metrics_per_sample:
                    all_keys.update(d.keys())
                for k in sorted(all_keys):
                    vals = torch.tensor(
                        [d.get(k, float("nan")) for d in fg_metrics_per_sample],
                        dtype=torch.float,
                    )
                ...
                for i in range(batch_size):
                    ...
                    if case_dir is not None:
                        dump_viser_artifact(
                            ...
                            data["cmaps"][i],
                            pred_extrinsics[i],
                            os.path.join(case_dir, "geometry_pred"),
                            max_pts=100_000,
                        )
                        dump_viser_artifact(
                            data["pmaps"][i],
                            ...
                            data["cmaps"][i],
                            data["extrinsics"][i],
                            os.path.join(case_dir, "geometry_gt"),
                            max_pts=100_000,
                        )

        # Visualize point clouds
        if (not self.skip_test_viz) and (
            self.current_epoch % self.val_viz_freq == (self.val_viz_freq - 1)
            or self.current_epoch == 0
        ):
            for i in range(batch_size):
    ...
    def visualize_image_grid(
        ...
        if S > 8:
            indices = torch.linspace(0, S - 1, 8).long()
    ...
    # configure_optimizers, setup, _load_pretrained_weights are inherited
    # from BaseProbeModule. The base class loads with strict=False so old
    # Exp-A checkpoints continue to work after adding new heads.
