# Extracting features for timestep 261, layer 1
# hidden_states shape: torch.Size([1, 17550, 3072])
# resolution = 17550 = T: (1 frame + 8 fps * 6 s / 4 downsample) * H: 480 px / 16 downsample * W: 720 px / 16 downsample
# feature dim = 3072
...
class OneStepCogVideoXPipeline(CogVideoXPipeline):
    """
    ...
    def __call__(
        ...
        # Forward pass through the transformer to extract features
...
def get_cogvideox_featurizer(
    model_id: str = "THUDM/CogVideoX-2b", null_prompt: str = ""
):
    """
    Build the full CogVideoXFeaturizer pipeline exactly **once** per (model_id, device).

    The first call incurs checkpoint loading; later calls return the
    very same CogVideoXFeaturizer object (modules & tensors already live in RAM/GPU).
    """
    return CogVideoXFeaturizer(
        model_id=model_id,
...
class CogVideoXFeaturizer:
    ...
    def __init__(self, model_id="THUDM/CogVideoX-2b", null_prompt=""):
        """
    ...
    def forward(
        ...
        """
        Extract features from specific layers of the CogVideoX transformer.
...
if __name__ == "__main__":
    ...
    # load first 81 images under data/DL3DV/DL3DV-10K/1K/0a1b7c20a92c43c6b8954b1ac909fb2f0fa8b2997b80604bc8bbec80a1cb2da3/images_8
    frame_folder = "data/DL3DV/DL3DV-raw/DL3DV-10K/1K/0a1b7c20a92c43c6b8954b1ac909fb2f0fa8b2997b80604bc8bbec80a1cb2da3/images_4"
    video = []
    for i in range(1, 50):
