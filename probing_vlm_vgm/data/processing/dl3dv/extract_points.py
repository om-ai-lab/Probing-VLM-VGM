...
from probing_vlm_vgm.vggt.visual_util import download_file_from_url, segment_sky
...
def process_scene(
    ...
    if num_frames > 0 and len(image_files) > num_frames:
        # Set deterministic seed based on hash_dir for reproducibility
...
def main():
    ...
    # Load VGGT model
    model = VGGT.from_pretrained("ckpt/VGGT-1B", local_files_only=True)
    model = model.to("cuda")
    model.eval()
