#!/usr/bin/env python3
"""
Extract one 81-frame feature window from a DL3DV scene using CogVideoX and
save **one safetensor per requested layer**:

    <out_dir>/feature_t{start_idx}_layer{layer_id}.sft

Each file holds a single key  ``"feat"``  whose value is shaped
**(T, H, W, C)** and stored in **FP16**.

Typical wrapper call
--------------------
python -m features.cogvideox.extract_features \
       --scene-dir   data/DL3DV/DL3DV-ALL-960P/1K/<hash>/images_4 \
       --data-sft    data/DL3DV/DL3DV-processed/1K/<hash>.sft \
       --out-dir     data/DL3DV/FEAT/cogvideox-t2v-5b/1K/<hash> \
       --model-id    ckpt/CogVideoX-5b \
       --t           749 \
       --output-layers 20
"""
from __future__ import annotations

import argparse
import glob
import logging
import math
import os
from typing import Dict, List

import torch
from PIL import Image
from safetensors.torch import load_file, save_file

from .cogvideox_feature import CogVideoXFeaturizer, get_cogvideox_featurizer
from .cogvideox_feature_i2v import get_cogvideox_featurizer_i2v

# ---------------------------------------------------------------------------- #
# Helpers                                                                      #
# ---------------------------------------------------------------------------- #


def list_frames(scene_dir: str, ext: str = "png") -> List[str]:
    """Return sorted list of frame PNG paths (frame_00001.png, …)."""
    return sorted(glob.glob(os.path.join(scene_dir, f"frame_*.{ext}")))


def load_frames(paths: List[str]) -> List[Image.Image]:
    return [Image.open(p) for p in paths]


def forward_cogvideox(
    model: CogVideoXFeaturizer,
    frames: List[Image.Image],
    t: int,
    layer_ids: List[int],
) -> Dict[int, torch.Tensor]:
    # 1.  Build 2 clips, each 49 frames:
    #     clip_0 = [0, 1, 3, 5, ..., 95]
    #     clip_1 = [0, 2, 4, 6, ..., 96]
    clip_indices = [
        [0] + [i + 2 * k for k in range(48)] for i in range(1, 3)  # 1≤i≤2
    ]  # list[2][49]

    # get the frames for each clip
    clips = [[frames[i] for i in indices] for indices in clip_indices]

    all_features = []
    for i, clip in enumerate(clips):
        if len(clip) != 49:
            raise RuntimeError(f"Clip {i} has {len(clip)} frames, expected 49.")

        with torch.no_grad():
            feat = model.forward(
                video=clip,
                t=t,
                output_layer_indices=layer_ids,
            )
            all_features.append(feat)

    # 2. Merge the feature dictionaries from both clips:
    merged_features = {
        layer_id: torch.cat(
            [all_features[0][layer_id], all_features[1][layer_id]], dim=0
        )
        for layer_id in all_features[0].keys()
    }
    return merged_features


# ---------------------------------------------------------------------------- #
# Main                                                                         #
# ---------------------------------------------------------------------------- #


def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="CogVideoX feature extractor (one window, one file per layer)",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--scene-dir", required=True, help="Image folder")
    parser.add_argument("--data-sft", required=True, help="Processed .sft file")
    parser.add_argument("--out-dir", required=True, help="Directory to save features")
    parser.add_argument("--image-ext", default="png", help="Image file extension")
    parser.add_argument(
        "--model-id", default="THUDM/CogVideoX-5b-I2V", help="CogVideoX model ID"
    )

    # CogVideoX-specific
    parser.add_argument("--t", type=int, default=749, choices=range(0, 1001))
    parser.add_argument(
        "--output-layers",
        nargs="+",
        type=int,
        default=[20],
        help="Transformer block indices to extract",
    )
    args, unknown = parser.parse_known_args(argv)

    # set up logging
    logging.basicConfig(
        level=logging.INFO,
        format="{asctime}: [{levelname}] {message}",
        style="{",
        datefmt="%Y-%m-%d %H:%M",
    )

    if unknown:
        logging.debug(f"[warn] ignored unknown args: {unknown}")

    out_dir = os.path.abspath(args.out_dir)
    os.makedirs(out_dir, exist_ok=True)

    # ------------------------------------------------------------------ #
    # 0 . Determine which layers are still missing (resume support)      #
    # ------------------------------------------------------------------ #
    fname_prefix = "feature"
    if not args.model_id.endswith("I2V"):
        model_size = args.model_id.split("-")[-1]
        fname_prefix = f"feature_t2v_{model_size}"
    missing_layers = [
        l
        for l in args.output_layers
        if not os.path.exists(
            os.path.join(out_dir, f"{fname_prefix}_t{args.t}_layer{l}.sft")
        )
    ]

    if not missing_layers:
        logging.info("All requested layer files already exist - nothing to do.")
        return

    # --------------------------------------------------------------------- #
    # 1 . Pick the temporal window                                          #
    # --------------------------------------------------------------------- #
    meta = load_file(args.data_sft)
    start_idx = int(meta["start_idx"].item())  # 0-based index ↔ frame_00001.png
    window_size = 49 * 2 - 1

    frame_paths = list_frames(args.scene_dir, ext=args.image_ext)
    total = len(frame_paths)

    if start_idx + window_size <= total:
        window_paths = frame_paths[start_idx : start_idx + window_size]
    elif start_idx == 0 and total < window_size:
        # Short scene: pad by repeating the last frame so CogVideoX sees a
        # full 97-frame window (2 interleaved clips of 49 frames each).
        # Query-GT alignment stays correct because dataset clamps
        # effective_context_len <= gt_num_frames <= total.
        pad = window_size - total
        logging.warning(
            f"[CogVideoX] {os.path.basename(args.scene_dir)}: only {total} "
            f"frames (< {window_size}); padding last frame x{pad}. "
            f"Last ~{math.ceil(pad / window_size * 100)}% of feature tokens "
            f"may contain padding artifacts."
        )
        window_paths = frame_paths + [frame_paths[-1]] * pad
    else:
        raise RuntimeError(
            f"start_idx={start_idx} + {window_size} > total={total} in "
            f"{args.scene_dir}; stale or inconsistent data_sft"
        )

    frames = load_frames(window_paths)

    # --------------------------------------------------------------------- #
    # 2 . Forward pass                                                     #
    # --------------------------------------------------------------------- #
    if args.model_id.endswith("I2V"):
        # Use the I2V model for image-to-video feature extraction
        cogvideox = get_cogvideox_featurizer_i2v(model_id=args.model_id)
    else:
        cogvideox = get_cogvideox_featurizer(model_id=args.model_id)

    feats = forward_cogvideox(
        cogvideox,
        frames,
        t=args.t,
        layer_ids=missing_layers,
    )

    # Warn about missing layers
    missing = set(missing_layers) - set(feats.keys())
    if missing:
        logging.warning(
            f"[warn] requested layers {sorted(missing)} not returned by CogVideoX model"
        )

    # --------------------------------------------------------------------- #
    # 3 . Save one .sft per layer                                           #
    # --------------------------------------------------------------------- #
    saved = 0
    for layer_id, feat in feats.items():
        out_path = os.path.join(
            out_dir, f"{fname_prefix}_t{args.t}_layer{layer_id}.sft"
        )
        save_file({"feat": feat.half()}, out_path)
        logging.debug(
            f"[CogVideoX] saved layer {layer_id} → {out_path} "
            f"shape {tuple(feat.shape)}"
        )
        saved += 1

    if saved == 0:
        logging.error("[err] no layers saved - aborting")
    else:
        logging.info(f"[CogVideoX] done - {saved} layer files written to {out_dir}")


if __name__ == "__main__":
    torch.set_float32_matmul_precision("high")
    main()
