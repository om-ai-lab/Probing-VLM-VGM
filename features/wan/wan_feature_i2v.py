...
class OneStepWanPipeline(WanImageToVideoPipeline):
    ...
    def __call__(
        ...
        """
        # `self._execution_device` can become CPU after manually offloading
        # auxiliary modules (text/image encoder). The VAE/transformer stay on
        # CUDA for extraction, so use the VAE parameter device as the source of
        # truth for tensors that enter the denoising path.
        ...
        ).to(device, dtype=self.vae.dtype)
        # The I2V image encoder is CLIP and is loaded in fp16, while the WAN
        # transformer/VAE run in bf16. The caller may wrap the whole pipeline in
        # bf16 autocast, so keep CLIP encoding out of autocast to avoid feeding
        # bf16 inputs to fp16 conv weights.
        self.image_encoder.to(device)
        with torch.amp.autocast("cuda", enabled=False):
            image_embeds = self.encode_image(image, device).to(self.transformer.dtype)
...
class WanFeaturizerI2V:
    ...
    def forward(
        ...
        Args:
            ...
            condition_frame (`str`): Which clip frame to use as I2V image condition.
                ``"first"`` matches the usual image-to-video setting; ``"last"`` is
        ...
        # Load and process the I2V condition image. Official I2V usage conditions
        # on a starting image, so "first" is the default for clean experiment
        # semantics. "last" remains available for backward compatibility.
        if condition_frame == "first":
            image = video[0]
        elif condition_frame == "last":
            image = video[-1]
        else:
            raise ValueError(f"condition_frame must be 'first' or 'last', got {condition_frame!r}")
        max_area = 480 * 832
