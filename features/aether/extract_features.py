...
def main(argv: list[str] | None = None) -> None:
    ...
    if start_idx + window_size <= total:
        ...
        # Short scene: pad by repeating the last frame so Aether sees a full
        # 81-frame window (2 interleaved clips of 41 frames each).
        # Query-GT alignment stays correct because dataset clamps
        # effective_context_len <= gt_num_frames <= total.
        pad = window_size - total
        logging.warning(
            f"[Aether] {os.path.basename(args.scene_dir)}: only {total} "
