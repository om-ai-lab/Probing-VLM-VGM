...
from probing_vlm_vgm.vggt.heads.head_act import activate_head
from probing_vlm_vgm.vggt.heads.utils import create_uv_grid, position_grid_to_embed
...
class DPTHead(nn.Module):
    ...
    def __init__(
        ...
        intermediate_layer_idx: List[int] = [2, 5, 8, 11],
        pos_embed: bool = True,
    ...
    def forward(
        ...
        Args:
            ...
            n_patches (tuple): Number of patches in the input (H, W).
            frames_shape (tuple): Shape of the input frames (B, S, 3, H, W).
        ...
        """
        S = frames_shape[1]  # Number of frames in the input sequence

        # If frames_chunk_size is not specified or greater than S, process all frames at once
        if frames_chunk_size is None or frames_chunk_size >= S:
            return self._forward_impl(
                aggregated_tokens_list, n_patches, frames_shape, patch_start_idx
            )
    ...
    def _forward_impl(
        ...
        assert (
            len(n_patches) == 2
        ), f"n_patches should be a tuple of (H, W), got {n_patches}"
        assert (
            len(frames_shape) == 5
        ), f"frames_shape should be a tuple of (B, S, 3, H, W), got {frames_shape}"
        B, S, _, H, W = frames_shape
        patch_h, patch_w = n_patches[0], n_patches[1]

        if frames_start_idx is not None and frames_end_idx is not None:
            assert frames_end_idx <= S and frames_start_idx < frames_end_idx
        ...
        out = custom_interpolate(
            out,
            (
                int(H / self.down_ratio),
                int(W / self.down_ratio),
            ),
            mode="bilinear",
            align_corners=True,
        )
