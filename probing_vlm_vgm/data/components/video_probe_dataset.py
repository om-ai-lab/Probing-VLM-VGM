...
from probing_vlm_vgm.vggt.utils.geometry import closed_form_inverse_se3, mat2plucker

logger = logging.getLogger(__name__)


def pool_vfm_feat_spatial(
    feat: torch.Tensor, target_hw: tuple
) -> torch.Tensor:
    """Spatial adaptive-avg-pool a VFM feature map (T, H, W, C) → (T, H', W', C).

    Used to equalize token counts across different VFMs (e.g. downsample WAN's
    30x52 feature map to match a VLM's 16x16 token grid) for controlled
    token-budget comparisons.

    Upsampling latents is not well-defined (adaptive_avg_pool2d would degenerate
    to near-nearest-neighbor replication), so this function asserts target <= source.
    """
    T, H, W, C = feat.shape
    ...
    assert target_hw[0] <= H and target_hw[1] <= W, (
        f"target {target_hw} must be <= source ({H},{W}); upsampling latents is "
        f"not well-defined for this downsampling-only helper."
    )
    feat = feat.permute(0, 3, 1, 2).contiguous()  # (T, C, H, W)
    feat = F.adaptive_avg_pool2d(feat, output_size=target_hw)
    return feat.permute(0, 2, 3, 1).contiguous()  # (T, H', W', C)
...
class VideoProbeDataset(EasyDataset):
    ...
    def __getitem__(self, idx):
        ...
        # get total_frames from shape without converting dtype
        total_frames = data["images"].shape[0]
        ...
        # use min(context_len, total_frames) to avoid index out of bounds
        effective_context_len = min(self.context_len, total_frames)
        ...
        sel = []
        sel_seen = (
            self._sample_query_frames(self._rng, n_seen, effective_context_len, 0)
            if n_seen > 0
            else None
        )
        unseen_start = (
            ...
            if n_unseen > 0 and total_frames > effective_context_len
            else 0
        )
        sel_unseen = (
        ...
        # Index FIRST, then .float() — avoids allocating float32 copies of all
        # S frames when only num_views are needed.
        images = data["images"][sel]  # (num_views, 3, H, W), keep original dtype
        pointmaps = data["pointmaps"][sel].float()  # (num_views, H, W, 3)
        confmaps = data["confmaps"][sel].float()  # (num_views, H, W, 1)
        depthmaps = data["depthmaps"][sel].float()  # (num_views, H, W, 1)
        intrinsics = data["intrinsic"][sel].float()  # (num_views, 3, 3)
        extrinsics = data["extrinsic"][sel].float()  # (num_views, 3, 4)
        masks = data["masks"][sel] if self.use_mask and "masks" in data else None
        ...
        if self.vfm_name in self.VIDEO_DIFFUSION_VFMS:
        ...
        elif self.vfm_name in self.CHUNKED_VIDEO_VFMS:
            ...
            # Parse layer indices from feat_postfix
            # e.g., "_layer32" -> [32], "_layer8_16_24_32" -> [8, 16, 24, 32]
            if "layer" in self.feat_postfix:
                ...
                layer_indices = [32]  # default to last layer
            
            # Load features for each layer
            vfm_feat_layers = []
            for layer_idx in layer_indices:
                layer_path = os.path.join(
                    os.path.dirname(vfm_feat_path),
                    f"feature_layer{layer_idx}.sft"
                )
            ...
            if len(vfm_feat_layers) == 1:
                ...
                vfm_feat = torch.cat(vfm_feat_layers, dim=-1)  # (T, H, W, C*num_layers)
            
            T = vfm_feat.shape[0]
            
            # Generate query frame sequence (must match extraction)
            # Rule: 0, 1, then 1 + k*divisor (k=1,2,...), until < context_len
            divisor = self.query_idx_divisor if self.query_idx_divisor else 4
            query_frame_seq = [0, 1]
            idx = 1 + divisor
            while idx < self.context_len:
                query_frame_seq.append(idx)
                idx += divisor
            query_frame_seq = query_frame_seq[:T]  # ensure length matches features
            
            # Build mapping from original frame index -> feature index
            frame_to_feat_idx = {frame: i for i, frame in enumerate(query_frame_seq)}
            
            # Map sel (query frame indices) to feature indices
            # sel values are guaranteed to be in query_frame_seq due to query_idx_divisor alignment
            vfm_idx = torch.tensor(
                [frame_to_feat_idx.get(s.item(), 0) for s in sel],
                dtype=torch.long,
            )
        ...
        # Optional: pool vfm feature spatially to match a target token grid
        # (for controlled token-budget comparisons between VFMs of different token counts)
        if self.target_spatial_size is not None:
            vfm_feat = pool_vfm_feat_spatial(vfm_feat, self.target_spatial_size)

        # transform and scale the point maps so the extrinsics of start is identity rotation and zero translation
        # (data already indexed by sel above)
        extrinsics, pointmaps, depthmaps = invert_pose_ref_and_scale(
            extrinsics,  # (num_views, 3, 4)
            pointmaps,  # (num_views, H, W, 3)
            depthmaps=depthmaps,  # (num_views, H, W, 1)
            ref_idx=0,
            scale_by_points=True,
        )

        image = images.float() / 255.0  # (num_views, 3, H, W)
        pointmap = pointmaps  # (num_views, H, W, 3)
        confmap = confmaps  # (num_views, H, W, 1)
        depthmap = depthmaps  # (num_views, H, W, 1)
        intrinsic = intrinsics  # (num_views, 3, 3)
        extrinsic = extrinsics  # (num_views, 3, 4)
        # masks already indexed by sel above
