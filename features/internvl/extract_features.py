#!/usr/bin/env python3
"""
Extract features from InternVL3 models for probe training.

This script extracts hidden states from specified layers of InternVL3 models
...
from .base_extractor import get_internvl_extractor
...
def parse_args(argv: List[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="InternVL feature extractor for probe training",
    ...
    parser.add_argument(
        ...
        help="Path to InternVL model (local or HuggingFace)",
    )
    parser.add_argument(
        "--model-type",
        choices=["internvl3", "internvl35", "sensenova"],
        default="internvl3",
        help=(
            "Model type: 'internvl3' for original InternVL3, "
            "'internvl35' for InternVL3.5 checkpoints, or 'sensenova' "
            "for spatial SI versions"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Maximum number of tiles per frame (1 recommended for video)",
    )
    parser.add_argument(
        "--output-layers",
        nargs="+",
        ...
        help=(
            ...
            "(InternVL3-8B has 28 layers; InternVL3.5-8B has 36 layers)"
        ),
    )
    parser.add_argument(
        "--prompt",
...
def main(argv: List[str] | None = None) -> None:
    ...
    logger.info(f"Device: {args.device}, Layers: {args.output_layers}")
    extractor = get_internvl_extractor(
