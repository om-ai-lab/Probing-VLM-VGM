...
def get_point_cloud(
    ...
    Args:
        ...
        masks_fg: foreground mask (same spatial size as pointmaps). If None, no mask filter.
    ...
    if masks_fg is not None:
        fg_mask = masks_fg.reshape(-1).astype(bool)
        conf_mask = conf_mask & fg_mask
    else:
        conf_mask = conf_mask
...
def visualize_sft(sft_data, output_dir):
    ...
    masks = sft_data.get("masks", None)  # (S, H, W, 1)
    ...
    vertices, colors = get_point_cloud(
        pointmaps, confmaps, images, masks, conf_thres=0.0, max_points=10000, fps=False
    )
    ...
    for i in range(num_views):
        ...
        if masks is not None:
            mask_uint8 = (masks[i].squeeze() * 255).astype(np.uint8)  # (H,W)
            mask_path = os.path.join(output_dir, f"masks/{i:05d}.png")
            Image.fromarray(mask_uint8, mode="L").save(mask_path)

            mask_2d = masks[i].squeeze()  # (H,W)  bool/0-1
            img_rgb = img.astype(np.uint8)  # (H,W,3) uint8
            masked_rgb = (img_rgb * mask_2d[..., None]).astype(np.uint8)
            masked_path = os.path.join(output_dir, f"masked_images/{i:05d}.png")
            Image.fromarray(masked_rgb, mode="RGB").save(masked_path)
...
# run in bash:
# for file in data/DL3DV/DL3DV-processed/1K/0*.sft; do hash=$(basename "$file" .sft); python -m probing_vlm_vgm.data.processing.visualize_sft --sft "$file" --output data/DL3DV/DL3DV-10K/test_output/"$hash"; done
