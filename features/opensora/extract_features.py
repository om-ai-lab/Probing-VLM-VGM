...
def main(argv: list[str] | None = None) -> None:
    ...
    if start_idx + window_size <= total:
        ...
        # Short scene: pad by repeating the last frame so Open-Sora sees a
        # full 129-frame window. Query-GT alignment stays correct because
        # dataset clamps effective_context_len <= gt_num_frames <= total.
        pad = window_size - total
        logging.warning(
            f"[Open-Sora] {os.path.basename(args.scene_dir)}: only {total} "
