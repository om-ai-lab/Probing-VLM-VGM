...
from probing_vlm_vgm.data.components.video_probe_dataset import VideoProbeDataset
from probing_vlm_vgm.data.components.scannet_instance_dataset import ScanNetInstanceDataset  # noqa: F401  (used by eval())
from probing_vlm_vgm.data.components.scannet_tagging_dataset import ScanNetTaggingDataset  # noqa: F401  (used by eval())
...
def get_data_loader(
    ...
    # prefetch_factor is per-worker BATCHES (not samples). Default 2 keeps
    # in-flight memory bounded: 8 workers × 2 × batch_size × ~42MB/sample ≈ 10GB
    # per loader (× 2 for persistent train+val). Raising it to absorb NFS jitter
    # is tempting but a 36MB feature × 1024 in-flight samples = ~43GB queue
    # that delays first-batch by minutes on cold cache. Pre-warm page cache or
    # stage to local NVMe instead of relying on a fat prefetch queue.
    # Only valid when num_workers > 0; PyTorch raises if set with num_workers=0.
    extra = {"prefetch_factor": 2} if num_workers > 0 else {}
...
class VideoProbeDataModule(LightningDataModule):
    ...
    def train_dataloader(self) -> DataLoader[Any]:
        ...
        self.train_loader = get_data_loader(
            ...
            else "forkserver",  # forkserver avoids the OpenMP/libgomp fork-deadlock that hits multi-process concurrent training (3+ jobs hanging at GPU 0% util). DeepSpeed needs spawn (default fork triggers "cannot allocate memory" on ZeRO-2).
        ...
        for dataset in val_datasets:
            ...
            val_loaders.append(
                get_data_loader(
                    ...
                    multiprocessing_context="spawn"
                    if isinstance(self.trainer.strategy, DeepSpeedStrategy)
                    else "forkserver",  # forkserver avoids the OpenMP/libgomp fork-deadlock that hits multi-process concurrent training (3+ jobs hanging at GPU 0% util). DeepSpeed needs spawn.
